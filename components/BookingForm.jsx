...
const BookingForm = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (
        departureCityParam &&
        europeanCities.some((city) => city.value === departureCityParam)
      ) {
      ...
      if (
        ...
        europeanCities.some((city) => city.value === destinationCityParam)
      ) {
  ...
  const validateForm = () => {
    ...
    if (formData.tripType === "roundTrip") {
      if (!formData.return) {
        ...
        newErrors.return =
          "Return date must be same or later than departure date";
        isValid = false;
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      const departureCityLabel =
        europeanCities.find((city) => city.value === formData.departureCity)
          ?.label || formData.departureCity;
      const destinationCityLabel =
        europeanCities.find((city) => city.value === formData.destinationCity)
          ?.label || formData.destinationCity;

      // Format dates for display
      const departureFormatted = formData.departure
        ? format(formData.departure, "yyyy-MM-dd")
        : "";
      const returnFormatted = formData.return
        ? format(formData.return, "yyyy-MM-dd")
        : "";
